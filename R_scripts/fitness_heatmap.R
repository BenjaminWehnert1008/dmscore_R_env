...
fitness_heatmap <- function(input_csv_path, threshold = 3, output_pdf_path, img_format = "pdf") {
  ...
  if (!"fitness" %in% colnames(heatmap_data)) {
    stop("The column 'fitness' is not found in the data.")
  }

  # Create heatmap_data_long by selecting necessary columns
  heatmap_data_long <- heatmap_data %>%
    select(mut_aa, position, fitness, wt_aa)  # Use 'fitness'

  # Find the smallest non-NA value in fitness column
  min_non_na_value <- min(heatmap_data_long$fitness, na.rm = TRUE)
  ...
  min_count <- min(heatmap_data_long$fitness, na.rm = TRUE)
  max_count <- max(heatmap_data_long$fitness, na.rm = TRUE)
  max_position <- max(heatmap_data$position)

  # Create the heatmap plot with explicit handling for positions > max_position
  heatmap_plot <- ggplot(heatmap_data_long, aes(x = wt_aa_pos, y = mut_aa, fill = fitness)) +
    scale_fill_gradientn(
      colours = c("#D73027", "#F0F0F0", "#4575B4"),   # From low = red to high = blue
      values = c(0, abs(min_count) / (abs(min_count) + max_count), 1), # Manually define proportions
      na.value = "grey35",         # Color for NA values
      limits = c(min_count, max_count)               # Define the range for the color scale
    ) +
    ...
    theme_minimal() +
    labs(title = "Heatmap of Fitness Estimations for Mutations", x = "Wild-type Amino Acid", y = "Mutant Amino Acid", fill = "Fitness", color = "Synonymous Mutation") +
...
fitness_heatmap(
  ...
  "/Users/benjaminwehnert/CRG/DMS_QC/howard/fitness_heatmap.pdf",
  threshold = 3
)
